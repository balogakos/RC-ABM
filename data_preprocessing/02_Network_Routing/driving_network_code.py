...
LSOA_PATH = os.path.join(INPUT_DIR, 'CA boundary.shp')
POSTCODE_PATH = os.path.join(INPUT_DIR, 'postcode_CA_new.shp')
RETAIL_PATH = os.path.join(INPUT_DIR, 'Liverpool_new_retail_centres_01-26(version).shp')
OUTPUT_FILE = os.path.join(PROCESSED_DIR, 'driving_results.parquet')
CACHE_FILE = os.path.join(CACHE_DIR, 'network_cache.pkl')
TEST_MODE = True # Set to True to only process the first 100 postcodes
...
def run_analysis():
    ...
    if G is None:
        ...
        print("Building road network from polygon...")
    ...
    # --- Test Mode Logic ---
    if TEST_MODE:
        print(f"TEST_MODE is ON. Processing only first 100 postcodes.")
        pcd_ids = pcd_ids[:100]
    ...
    for i in range(0, total_sources, chunk_size):
        ...
        for j, row_dists in enumerate(dists_matrix):
            ...
            # REMOVED LIMIT: Return all reachable retail centers
